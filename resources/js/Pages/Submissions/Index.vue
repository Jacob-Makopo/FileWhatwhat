...
const props = defineProps({ filters: Object, submissions: Object })
</script>
...
<template>
  <AppLayout>
    ...
    <div v-if="!submissions?.data?.length" class="mt-6 rounded-xl border bg-white p-6 text-slate-600">
      No submissions yet.
    </div>
      ...
      <table class="min-w-full text-sm">
        ...
        <tbody>
          <tr v-for="s in submissions.data" :key="s.id" class="border-t">
            ...
            <td class="px-4 py-3">{{ new Intl.NumberFormat('en-ZA',{style:'currency',currency:'ZAR'}).format(s.amount) }}</td>
